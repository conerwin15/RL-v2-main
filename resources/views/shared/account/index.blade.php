...
<form method="POST" class="mt-5" id="upload_picture_form" enctype="multipart/form-data" >
    @csrf
            ...
            <div class="ml-3">
                <p><b>{{ $user->name }} </b> <span class="btn-theme btn-sm ">{{ucfirst( toRoleLabel($user->roles[0]->name ))}}</span> <br>
                    ...
                    {{ __('lang.joined-on') }} {{ $user->created_at->format('M Y')}}
                        @if($routeSlug == 'staff')
                            | Under : {{ucfirst($user->getNameById($user->dealer_id)) }}
                        @endif
                    </small>
                </p>
            </div>
        ...
        <div class="row">
            <div class="col-sm-6">
             ...
             <div class="user-label">{{ $user->country ? $user->country->name : 'N/A' }}</div>
                ...
                <div class="d-flex">
                    <label>{{ __('lang.points') }}:</label>
                    <div class="user-label">
                        @if(count($user->totalPoints) > 0)
                            {{ $user->totalPoints[0]->totalPoints }}
                        @else
                            0
                        @endif
                    </div>
                ...
                <div class="d-flex">
                    @if($routeSlug == 'staff')
                      <label>{{ __('lang.dealer') }}:</label>
                      <div class="user-label">{{ucfirst($user->getNameById($user->dealer_id)) }}</div>
                    @else
                       <label>{{ __('lang.created-by') }}:</label>
                      <div class="user-label">{{ ucfirst($user->createdBy->name) }}</div>
                    @endif
                </div>
            ...
            <div class="col-sm-6">
                ...
                <div class="d-flex">
                    ...
                    <div class="user-label">
                        {{ $user->jobRole  ?  $user->jobRole->name : 'N/A' }}
                    </div>
                </div>
...
@if(count($userLearningPaths) > 0)
    <div class="wrapper2">
        <div class="container-fluid max-width">
            <p class="color">{{ __('lang.rewards') }}</p>
        </div>
    </div>
    <div class="white-wrapper wrapper2">
        ...
        <div class="container-fluid max-width">
            ...
            </div>@foreach($userLearningPaths as $userLearningPath)
            <div class="row">
                <div class="col-md-6">
        ...
        <div class="modal-content">
            <div class="modal-body">
                ...
                <form action="{{ url($routeSlug . '/change-password/') }}" method="POST"
                    id="changePasswordForm" class="mt-5">
                    @csrf
