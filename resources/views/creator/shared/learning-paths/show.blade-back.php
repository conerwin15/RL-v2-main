...
<div id="pathAlert"></div>
   ...
   <!--  <div class="row" style="margin-left: 1%">
        ...
        <a class="btn btn-primary btn-sm active" href="{{route('learning-paths.show',$courses[0]->id)}}"> Courses </a> &nbsp;
        <a class="btn btn-primary btn-sm" href="{{route('learners.show',$courses[0]->id)}}">Learners</a>
            ...
            @if(count($courses)>0)
             @foreach ($courses[0]->courses as $course)
                <tr>
                    <td>{{++ $index}}</td>
                    <td>{{$course->name}}</td>
                    <td>{{$user->getNameById($course->pivot->created_by)}}</td>
                    <td>{{date('d M Y', strtotime($course->pivot->created_at))}}</td>
                    <td>
