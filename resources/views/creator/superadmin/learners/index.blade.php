...
<div id="pathAlert"></div> 
            ...
            @if(count($courses)>0)
             @foreach ($courses[0]->courses as $course)
                <tr>
                    <td>{{++ $index}}</td>
                    <td>{{$course->name}}</td>
                    <td>{{$user->getNameById($users->pivot->created_by)}}</td>
                    ...
                    <form action="{{route('courses.destroy',$course->id)}}" method="POST">
                        ...
                        @method('DELETE')
                         ...
                         <button type="submit" onclick="return confirm('{{ __('lang.delete-confirmation-text') }}')" class="btn btn-danger btn-sm" style="background-color: #e3342f;border-color: #e3342f;"><!-- <img src="{{url('images/search_icon.png')}}"> -->{{__('lang.remove-from-path')}}
                         </button>
    ...
    <div class="modal-dialog">
      ...
      <!-- Modal content-->
        ...
        <!-- form -->
        <form action="{{ url('/superadmin/courses') }}" method="POST" >
        @csrf
        <div class="row">
