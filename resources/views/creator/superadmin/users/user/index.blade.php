...
<div class="dash-title container-fluid">
    ...
    <div class="dropdown custom-drop">
        <button type="button" style="background:#388fb5 !important;border-radius:4px;color:#fff;min-width:170px" class="btn-theme dropdown-toggle  {{ request()->is('users') ? 'active' : '' }}" id="dropdownMenuOffset" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" data-offset="10,20">
            {{__('lang.add-new-user')}}
            <svg viewBox="0 0 22.243 12.621" style="width:10px;margin-right:-5px;">
                <path d="M26.5,11.5l-9,9-9-9" transform="translate(-6.379 -9.379)" style="stroke:#fff !important;fill:none" stroke-linecap="round" stroke-linejoin="round" stroke-miterlimit="10" stroke-width="3" />
            </svg>
        </button>
        <div class="dropdown-menu title-menu" aria-labelledby="dropdownMenuOffset">
            <!-- <li><a class="dropdown-item" href="{{ url('/superadmin/users') }}">View Users</a></li> -->
...
<br>
<a  style="float:right;" class="" href="{{ url('/superadmin/learning-progress/export?type=excel&country='.$export_countries. '&region='. $export_regions . '&group=' . $export_groups . '&jobRole=' . $export_jobRoles. '&role=' . $export_roles) }}">
        ...
        <div class="row align-items-end">
            ...
            <div class="col-sm-2">
                ...
                <select name="filter_jobRole" class="form-control select" required>
                    ...
                    <option value="{{ $jobRole->id }}" id="roleId" {{ @$_GET['filter_jobRole'] == $jobRole->id ? 'selected' : '' }}>
            ...
            <div class="col-sm-2">
                ...
                <select name="filter_role" class="form-control select" required>
                    ...
                    <option value="{{ $role->id }}" id="roleId" {{ @$_GET['filter_role'] == $role->id ? 'selected' : '' }}>
            ...
            <div class="col-sm-2">
                ...
                <select name="filter_group" class="form-control select" required>
                    ...
                    <option value="{{ $group->id }}" id="roleId" {{ @$_GET['filter_group'] == $group->id ? 'selected' : '' }}>
                        {{ ucfirst($group->name) }}
                    </option>
                    @endforeach
                </select>
...
<div class="container-fluid">
    <div class="white-wrapper">
        <form class="d-lg-flex justify-content-end align-items-center" method="GET">
            ...
            <input type="text" placeholder="{{__('lang.search-by-name-placeholder')}}" class="form-controller form-control mb-0" id="search" name="name" value="{{isset($_GET['name']) ? $_GET['name'] : ''}}">
            <button type="submit" class="ml-2 btn-theme">{{__('lang.search')}}</button>
        </form>
        <div class="table mt-4">
            <table  class="data-table display">
            <h6 class="col-3"><b>  {{__('lang.users')}} </b></h6>
                <thead>
                    <tr>
                        <th>{{__('lang.no')}}</th>
                        <th>{{ __('lang.name') }}</th>
...
<script>
    ...
    $(document).ready(function() {
        ...
        var ajaxUrl = "{{url('superadmin/users')}}" + window.location.search;
        var table = $('.data-table').DataTable({
            ...
            "createdRow":  function( row ) {
                $(row).find('td:eq(11)').addClass('flex');
            }
        });
    });
</script>
