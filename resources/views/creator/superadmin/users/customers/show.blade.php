...
<div class="white-wrapper  profile-wrap wrapper2">
    ...
    <div class="container-fluid max-width">
            ...
            <div class="ml-3">
                <p><b>{{ $user->name }} </b> <span class="btn-theme btn-sm ">{{ count($user->roles) > 0 ? ucfirst( toRoleLabel($user->roles[0]->name)) : ''}}</span> <br>
                    <small class="text-gray">
                    {{ __('lang.joined-on') }} {{ $user->created_at->format('M Y')}} |
                    {{ __('lang.under') }}: <strong>{{ $user->dealer ? $user->dealer->name : 'N/A'}}</strong>
                    </small>
                </p>
            </div>
        </div>
    ...
    <div class="row">
        <div class="col-sm-6">
            <div class="d-flex">
                ...
                <div class="user-label">{{ $user->name ?? 'N/A' }}</div>
            </div>

            <div class="d-flex">
                <label>{{ __('lang.country') }}:</label>
                <div class="user-label">{{ $user->country ? $user->country->name : 'N/A' }}</div>
            </div>
            ...
            <div class="d-flex">
                ...
                <div class="user-label">{{ $user->group ? $user->group->name : 'N/A' }}</div>
            </div>

            <div class="d-flex">
                <label>{{ __('lang.remarks') }}:</label>
                <div class="user-label">{{ $user->remarks ?? 'N/A' }}</div>
            </div>
        </div>
        ...
        <div class="col-sm-6">
            <div class="d-flex">
                ...
                <div class="user-label">{{ $user->email ?? 'N/A' }}</div>
            </div>

            <div class="d-flex">
                <label>{{ __('lang.job-role') }}:</label>
                <div class="user-label">{{ $user->jobRole ? $user->jobRole->name : 'N/A' }}</div>
            </div>

            <div class="d-flex">
                <label>{{ __('lang.role') }}:</label>
                <div class="user-label">
                    {{ count($user->roles) > 0 ? ucfirst(toRoleLabel($user->roles[0]->name)) : 'N/A' }}
                </div>
            </div>

            <div class="d-flex">
                <label>{{ __('lang.created-on') }}:</label>
                <div class="user-label">{{ $user->created_at ? date('d M Y', strtotime($user->created_at)) : 'N/A' }}</div>
            </div>
        </div>
    </div>
</div>
...
<div class="white-wrapper wrapper2">
        ...
        <div class="">
            ...
            <div class="row">
                <div class="col-md-6">
                    <img src="{{asset('storage' . Config::get('constant.LEARNING_PATH_STORAGE').'/'.$userLearningPath->learningPath->featured_image) }}" style="width:50px;">
