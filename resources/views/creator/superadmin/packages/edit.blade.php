...
<div class="dash-title container-fluid mt-2">
    <div>
        ...
        <span class="bradcrumb">{{ __('lang.edit-package') }}</span>
    </div>
</div>

<form action="{{ url('/superadmin/packages/' . $package->id) }}" method="POST" enctype="multipart/form-data" >
    @csrf
    @method('PUT')
                    ...
                    <option  disabled> {{ __('lang.select') }}  {{ __('lang.sub-category') }} </option>
                        @foreach($subCategories as $subCategory)
                            <option value="{{ $subCategory->id }}" id="jobRoleId"
                                {{ $package->sub_category_id == $subCategory->id ? "selected" : '' }}>
                                {{ $subCategory->name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="row">
                ...
                <div class="col-12 col-sm-8">
                    <div class="form-group">
                        ...
                        <textarea class="form-control descriptionText" rows="4" name="description" placeholder="{{ __('lang.description') }}" maxlength = "1200" required> {{$package->description}}</textarea>
                ...
                <!-- <div class="row"> -->
                    ...
                    <div class="form-group col-sm-5">
                        <label><strong>{{__('lang.image-preview')}}: </strong></label>
                        <div><img src="{{($package->image!= null) ? asset('storage' . Config::get('constant.LEARNING_PACKAGE') . $package->image) : asset('assets/images/avatar_default.png')}}" alt="image" style="width: 50%;"></div>
                    </div>
                <!-- </div> -->
                <div class="col-xs-12 col-sm-8">
                    <div class="form-group">
                ...
                <div class="col-xs-12 col-sm-8">
                    <div class="form-group">
                        ...
                        <input type="text" name="discounted_price" class="form-control txtPrice" id="name"
                            ...
                            value="{{ $package->discount_price }}" required>
