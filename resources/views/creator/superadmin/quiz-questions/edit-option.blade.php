...
<div class="dash-title container-fluid mt-2">
    <div>
        ...
        <span class="bradcrumb">{{ __('lang.edit-option') }}</span>
    </div>
</div>
...
<div class="container-fluid">
    <div class="white-wrapper ">
        ...
        <form action="{{ url('/superadmin/quiz-question/option/update') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="row">
                ...
                <div class="col-12 col-md-7">
                    ...
                    <textarea class="form-control" rows="4" name="text" placeholder="{{ __('lang.text') }}" maxlength="300">{{ $option->option_text }}</textarea>
                    @if($errors->has('text'))
            ...
            <div class="row">
                    ...
                    <div id="preview">
                        @if($option->media != '')
                                ...
                                @if(in_array(strtolower($ext[1]), $imageFormat))
                                    <img src="{{ asset('storage' . $optionViewPath . $option->media) }}"
                                        style='width:315px;height:150px'> &nbsp; &nbsp; &nbsp;
                                @else
                                    <video controls style="width: 100%;">
                                        <source
                                            src="{{ asset('storage' . $optionViewPath . $option->media) }}"
                                            type="video/mp4">
                        ...
                        <div class="row">
                            ...
                            <label>{{ __('lang.is-correct') }}:</label>
                                <select name="status" class="select form-control"  required>
                                    <option  value="1" {{ $option->is_correct == 1 ? "selected" : "" }}> {{ __('lang.yes') }} </option>
                                    <option value="0" {{ $option->is_correct == 0 ? "selected" : ""}}> {{ __('lang.no') }} </option>
                                </select>
                            </div>
                        </div>
                ...
                <div class="col-xs-12 col-sm-12 col-md-12">
                    <button type="submit" class="btn-theme">{{ __('lang.edit-option') }}</button>
                </div>
