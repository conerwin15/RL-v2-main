...
<div class="dash-title container-fluid max-width">
    ...
    <a class="btn-theme" href="{{ url('superadmin/sales-tips/create') }}">+ {{__('lang.create-sales-tip')}}</a>
</div>  
        ...
        <div class="box-col">
                ...
                <div class="bottom-text" >
                    ...
                    <!-- <div class="flex justify-content-start">
                        <small class="text-left">{{ __('lang.region') }}:
                            {{ $salesTip->region ? $salesTip->region->name : 'N/A' }}</small>
                        <small class="text-left pl-1 ml-1" style="border-left:1px solid #ccc">
                            {{ $salesTip->country ? $salesTip->country->name : 'N/A' }}</small>
                        <small class="text-left pl-1 ml-1" style="border-left:1px solid #ccc"> {{ __('lang.region') }}:
                            {{ $salesTip->region ? $salesTip->region->name : 'N/A' }}</small>
                        <small class="text-left pl-1 ml-1" style="border-left:1px solid #ccc">{{ __('lang.job-role') }}:
                            {{ $salesTip->JobRole ? $salesTip->JobRole->name : 'N/A' }}</small>
                        <small class="text-left pl-1 ml-1" style="border-left:1px solid #ccc">{{ __('lang.group') }}:
                            {{ $salesTip->group ? $salesTip->group->name : 'N/A' }}</small>
                        <br>
                        
                        </div> -->
                        <div class="text-right actionbuttons">
                            <a
                                href="{{ url('superadmin/sales-tips', $salesTip->id) }}"><i
                                    class="fa fa-eye" aria-hidden="true"></i> {{ __('lang.view') }}</a>
                            <form
                                action="{{ url('superadmin/sales-tips/' . $salesTip->id . '/edit') }}"
                                method="GET" class="text-right mb-0">
                                @csrf
                                <button type="submit" class="text-danger"
                                    style="border: 0px; background:transparent"><i class="fa fa-pencil"
                                        aria-hidden="true"></i> {{ __('lang.edit') }}
                                </button>
                            </form>

                            <form
                                action="{{ url('superadmin/sales-tips', $salesTip->id) }}"
                                method="POST" class="text-right mb-0">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="text-danger "
                                onclick="return confirm('{{ __('lang.delete-confirmation-text') }}')"
                                    style="border: 0px; background:transparent"><i class="fa fa-trash"
                                        aria-hidden="true"></i> {{ __('lang.delete') }}
                                </button>
                            </form>
                        </div>
                    </div>
...
{!! $salesTips->appends(request()->query())->links('vendor.pagination.bootstrap-4') !!}
