...
<div class="piaggio-alert">
    ...
    <div class="dash-title container-fluid">
        <div>
        <a href="{{ url('/admin/scheduled/mails') }}"><b>{{__('lang.manual-email')}}  &gt;</b></a>
           <span class="bradcrumb">{{$scheduleMailJobs->name}} &gt; {{__('lang.edit') }}</span>
        </div>
    </div>
    ...
    <br>
    <form action="{{url('superadmin/schedule/mail/update')}}" method="POST" class="container-fluid" id="frm-manual">
        @csrf
                        ...
                        <thead>
                            <tr>
                                <th width="12%" >
                                    ...
                                    <input type="hidden" id="existing_learners" name="existing_learners" value="{{$userIds}}"> 
            ...
            <input type="hidden" name="schedule_id" value="{{$scheduleMailJobs->id}}">
            <input type="hidden" name="frequency_type" id="frequency_type" value="{{$scheduleMailJobs->frequency}}">
            <div class="col-12 col-md-7">
                <div class="form-group">
                    <label>{{ __('lang.campaign-name') }}:</label>
                    <input class="form-control" name="campaign" placeholder="{{ __('lang.campaign') }}" value="{{$scheduleMailJobs->name}}">
            ...
            <div class="col-sm-4 mb-3">
                ...
                <label>{{ __('lang.recurrence') }}:</label>
                    <select name="recurrence" class="select form-control recurrence" required>
                        <option value="once" {{($scheduleMailJobs->frequency == 'once') ? 'selected' : ''}} disabled> {{ __('lang.once') }} </option>
                        <option  value="every" {{($scheduleMailJobs->frequency == 'every') ? 'selected' : ''}} disabled > {{ __('lang.repeat') }}</option>
                    </select>
                </div>
            </div>

            @if($scheduleMailJobs->frequency == 'every')
                <div class="col-sm-8">
                    <div class="row">
                        <div class="col-sm-6 mb-3 frequency_unit">
                            <div class="form-group">
                                <label>{{ __('lang.every') }} X:</label>
                                <select name="unit" class="select form-control"  required>
                                    <option value="day" {{($scheduleMailJobs->frequency_unit == 'day') ? 'selected' : ''}} > {{ __('lang.day') }}(s)</option>
                                    <option value="week" {{($scheduleMailJobs->frequency_unit == 'week') ? 'selected' : ''}} > {{ __('lang.week') }}(s) </option>
                                    <option value="month" {{($scheduleMailJobs->frequency_unit == 'month') ? 'selected' : ''}}> {{ __('lang.month') }}(s) </option>
                                </select>
                            </div>
                        </div>

                        <div class="col-sm-6 mb-3 frequency_amount">
                        <div class="form-group">
                        <label>X:</label>
                        <input type="number" class="form-control frequencyAmt" name="frequency_amt" placeholder="{{ __('lang.for-x-times') }}" value="{{$scheduleMailJobs->frequency_amount}}">
                ...
                <div class="row">
                    <div class="col-6 col-md-4">
                        <div class="form-group">
                            ...
                            <input type="datetime-local" class="form-control startDate" name="start_date" placeholder="{{ __('lang.start date') }}" min="{{$currentDate}}" value="{{$startDate}}">
                    ...
                    @if($scheduleMailJobs->frequency == 'every')
                        <div class="col-6 col-md-4 end_date">
                            <div class="form-group">
                                <label>{{ __('lang.end-date') }}:</label>
                                <input type="date" class="form-control endDate" name="end_date" min="{{$endDate}}" value="{{$endRunAt}}">
            ...
            <div class="col-12 col-md-7">
                ...
                <div class="form-group">
                    ...
                    <input class="form-control" name="subject" placeholder="{{ __('lang.subject') }}" value="{{$scheduleMailJobs->subject}}" required>
            ...
            <div class="col-12 col-md-7">
                <div class="form-group">
                    ...
                    <textarea class="form-control" rows="4" name="description" placeholder="{{ __('lang.description') }}" maxlength = "1200" required> {{$scheduleMailJobs->description}}</textarea>
    ...
    <script>
        ...
        $(document).ready(function (){
            ...
            var ajaxUrl = "{{url('admin/scheduled/mail')}}" + '/' + Id + '/edit';
            ...
            formData.push({ name: "new_learners", value: Id_selected });
            formData.push({name: 'removed_learners', value: removedId});
            var ajaxurl = app_url + "/admin/schedule/mail/update" ;
            $.ajax({
                url: ajaxurl,
                type:'POST',
                data:formData,
                success:function(data) {
                    if(data.success === true){
                        ...
                        $('body').find('.piaggio-alert .alert').css('animation' , 'alertIN ease-in-out .35s forwards , alertOut ease-in-out .35s 4s forwards')
                            setTimeout(() => {
                            window.location.href = app_url + "/admin/scheduled/mails";
                            }, 2000);
