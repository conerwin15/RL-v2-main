...
<div class="dash-title container-fluid">
    <div>
        <a  href="{{ url('/admin/news-promotions') }}"><b>{{__('lang.news-promotions')}} ></b></a> 
        <span class="bradcrumb">{{ __('lang.edit-news-promotions') }}</span>
    </div>
</div>

<form action="{{ url('/admin/news-promotions/' . $newsPromotion->id) }}" method="POST"
    enctype="multipart/form-data" id="add-news-promotions" class="container-fluid">
    @csrf
    @method('PUT')
    <input type="hidden" name="id" value="{{ $newsPromotion->id }}">
            ...
            <div class="row">
                <div class="col-sm-12 mb-3">
                    ...
                    <input type="text" name="title" class="form-control"
                        placeholder="{{ __('lang.title') }}" value="{{ $newsPromotion->title }}"
                        required>
                    @if($errors->has('title'))
                ...
                <div class="col-sm-12 mb-3">
                    ...
                    <textarea class=" form-control" rows="4" name="description" id="description"
                        required>{{ $newsPromotion->description }}</textarea>
                ...
                <div class="col-sm-6 mb-3">
                    ...
                    @if($newsPromotion->media != NULL)    
                        @php   $ext = explode('.', $newsPromotion->media); @endphp       
                        @if(in_array(strtolower($ext[1]), $imageFormat))
                            <img src="{{ asset('storage' . $viewStoragePath . $newsPromotion->media) }}"
                                style='width:200px;height:150px'> &nbsp; &nbsp; &nbsp;
                        @elseif(in_array(strtolower($ext[1]), $videoFormat))
                            <video controls>
                                <source
                                    src="{{ asset('storage' . $viewStoragePath . $newsPromotion->media) }}"
                                    type="video/mp4">
                                    {{ __('lang.video-support-message') }}
                            </video>

                        @else
                            <a href="{!! url('/admin/news-promotions/'. $newsPromotion->id . '/attachment') !!}"
                ...
                <div class="col-sm-6 mb-3">
                    ...
                    <select id="jobrole" name="filter_jobrole" class="select form-control">
                        ...
                        @foreach($jobRoles as $jobRole)
                            <option value="{{ $jobRole->id }}" {{$newsPromotion->job_role_id == $jobRole->id ? "selected" : ''}}>{{ $jobRole->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="col-sm-6  mb-3">
                        ...
                        <select id="region" name="region_id"  class="select form-control">
                            ...
                            @foreach ($regions as $region)
                                <option value="{{$region->id}}" {{ ($region->id == $newsPromotion->region_id) ? 'selected' : ''}}>{{ $region->name }}</option>
                            @endforeach
                        </select>
                </div>
                ...
                <div class="col-sm-6 mb-3">
                    ...
                    <select id="groups" name="filter_group" class="select form-control">
                        ...
                        @foreach($groups as $group)
                            <option value="{{ $group->id }}" {{$newsPromotion->group_id == $group->id ? "selected" : ''}}>{{ $group->name }}</option>
                        @endforeach
                    </select>
                </div>
