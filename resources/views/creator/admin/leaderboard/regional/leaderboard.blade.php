...
<div class="container-fluid max-width">
                    ...
                    <div class="emp-details">
                        <div class="basic-details">
                            <div class="flex justify-content-between">
                                <p class="mb-0"><b>{{$EmpOfTheMonth->user->name}}</b></p>
                                <div class="flex">
                                    <a href = "{{ url('/admin/regional/mark-featured/' . $EmpOfTheMonth->user->id . '/edit') }}"> {{ __('lang.edit-featured-content') }} </a>
                                    <form action="{{ url('/admin/mark-featured') }}" method="POST" class="mb-0 ml-2">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="color btn-nobg" onclick="return confirm('{{ __('lang.delete-confirmation-text') }}')"> {{ __('lang.remove-featured') }}</button>
                                    </form>
                                </div>
                            </div>
                            <p class="flex align-items-center justify-content-start mb-2">
                                ...
                                <!--  | {{$EmpOfTheMonth->user->region->name}} | {{ ucfirst($EmpOfTheMonth->user->getRoleNames()->first()) }} -->
                            </p>
                            <!-- <p>{{date("M d,Y " ,  strtotime($EmpOfTheMonth->updated_at))}}</p> -->
    ...
    <div class="flex justify-content-end mt-4">
            <form  method="GET" class="flex mr-2">
                    @if(isset($_GET['name']))
                        <input type="text" placeholder="{{__('lang.search-by-name-placeholder')}}" class="form-controller form-control mr-2" id="search" name="name" value="{{ $_GET['name'] }}" style="min-width:300px !important">
                    @else
                        <input type="text" placeholder="{{__('lang.search-by-name-placeholder')}}" class="form-controller form-control mr-2" id="search" name="name" style="min-width:300px!important">
                    @endif
                    <button type="submit" class="btn-theme">{{ __('lang.search') }}</button>
            </form>
            <div class="flex">
                <a href="{{url('/admin/regional/manage-points')}}"   class="btn-theme mr-2">{{ __('lang.manage-points-bulk') }} </a>
                <a href="{{url('/admin/regional/featured-records')}}" class="btn-theme">{{ __('lang.featured-record') }} </a>
            </div>
        </div>

    <div class="white-wrapper mt-2">
        <h5 class="bg-blue" style="margin: -15px -15px  16px -15px">{{ __('lang.organisation-leaderboard') }}</h5>

        <div class="container-fluid max-width">
            <table class="table leaderboard-table data-table">
                    ...
                    <thead>
                        <tr>
                            ...
                            <th width="140px">{{ __('lang.action') }}</th>
                        </tr>
                    </thead>
...
<script>
    /*********** datatable ***********/
        $(document).ready(function() {
            ...
            var ajaxUrl = "{{url('admin/regional/leaderboard')}}" + window.location.search;
            var table = $('.data-table').DataTable({
                ...
                "createdRow":  function(row) {
                    $(row).find('td:eq(5)').addClass('d-flex  white-space');
                }
            });
        });
    </script>
